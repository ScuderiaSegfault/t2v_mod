...
and then select `Partition OTA` in the menu `ESP-Hosted Slave OTA Config > Select OTA Method`.

Save (kbd:[S]) and close (kbd:[Q]) `menuconfig` and start building the P4 firmware.
...
(To leave the monitor use kbd:[Ctrl,\]])